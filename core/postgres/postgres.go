...
func (d *Database) GetUserByID(ctx context.Context, id string) (*models.UserDetails, error) {
	...
	row := d.db.QueryRowContext(ctx, "SELECT id,first_name,last_name,email_address,created_at,deleted_at,merged_at,parent_user_id FROM users WHERE id = $1;", id)
...
func (d *Database) GetAllUsers(ctx context.Context) ([]*models.UserDetails, error) {
	var userDetails []*models.UserDetails
	rows, err := d.db.QueryContext(ctx, "SELECT id,first_name,last_name,email_address,created_at,deleted_at,merged_at,parent_user_id FROM users")
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, err
		}
...
func (d *Database) ListUsers(ctx context.Context, limit, offset int64) ([]*models.UserDetails, error) {
	...
	rows, err := d.db.QueryContext(ctx, "SELECT id,first_name,last_name,email_address,created_at,deleted_at,merged_at,parent_user_id FROM users ORDER BY id LIMIT $1 OFFSET $2;", limit, offset)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, ErrNoData
		}
...
func (d *Database) DeleteUser(ctx context.Context, id string) error {
	_, err := d.db.ExecContext(ctx, "DELETE FROM users WHERE id = $1;", id)
	if err != nil {
		return err
	}
